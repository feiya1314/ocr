...
import cn.easyocr.ai.chat.service.resp.OpenAiChatResp;
 ...
 * @author : feiya
 * @date : 2023/5/21
 * @description : 调用openai的key请求
 */
@Component(ChatSourceKey.OPEN_AI)
@Slf4j
public class OpenAiChatImpl extends AbsAiChatService<AiChatMsgResp> {
    @Autowired
    ...
    protected StreamingResponseBody callChatApi(ChatContext chatContext) {
        ...
        Request request = new Request.Builder()
                .url(config.getChatGpt().getUrl())
                .headers(headers)
                .post(okHttpReqBody)
                .build();
        log.info("call openai for response");
    ...
    protected ChatStreamingEvent<AiChatMsgResp> streamRespToEvent(ChatContext chatContext, String StreamingRespData) {
        ...
        OpenAiChatResp response = JsonUtils.jsonToBean(StreamingRespData, OpenAiChatResp.class);
